...
Summary: Deployment/Journals

Anthony has put the finishing touches to the deployment branch, and we hope that it works by time Galvanize gets it fixed. As this is our last day, I suggested that we make an issue for our journals. This is so we can post what we have one at a time and then merge it to main.
...
We began deployment. Because I was on a Mac I tried to deploy the database, but my computer was running into issues. Anthony, who was on Windows, took over. We had a lot of struggles as there was something wrong on Galvanize end, so we had to make do with what we had. It took a lot of troubleshooting and waiting around but we are one step closer to deployment.

## Mar 20, 2024

Driver: Anthony

Summary: Design Change/Modal Log In

We started today with Anthony showing us how he changed the onboarding page and spent some time changing it to look better. Next, we changed the login functionality so that instead of a web page it will be a modal. It was hard, as it is different from the modal warning to delete. Finally, Anthony and I changed the design of the plant log detail page.

## Mar 19, 2024

Driver: Lyle/Chris/Anthony

Summary: Finished README/Clean up Code/ Design Change

Today, I worked on finishing the README.md. Once again we used Smelli-Belli as a reference. Chris took over and did error handling. He made it so that if a user goes to a plant that was made by a different user, they can’t edit or make a plant log for that plant. We used the last remaining time working on the design of the onboard page, thanks to Anthony. At this point our MVP is done and we are working on making the webpage look nicer.
