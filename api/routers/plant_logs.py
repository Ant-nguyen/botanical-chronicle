...
def create_plant_log(
    ...
    if plant["account_id"] != account_data["id"]:
        raise HTTPException(status_code=403, detail="Logged in user does not match plant account id")
...
@router.put("/api/plants/plants-logs/{plant_log_id}", response_model=PlantLog)
def update_plant_log(
    ...
    return queries.update_plant_log(plant_log_id=plant_log_id, plant_log_in=plant_log_in)


@router.delete("/api/plants/plants-logs/{plant_log_id}", response_model=DeleteStatus)
